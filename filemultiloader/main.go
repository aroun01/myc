...
func getTasks() ([]Task, error) {
	url := "http://taskman:4001/api/tasks/get_list"
...
func setStatusTask(itemUserID int, itemUrl, itemID, itemStatus string) {
	// cmd := "ls -l"
	cmd := "curl -XPOST http://taskman:4001/api/tasks/set_task -H 'Content-Type: application/json' -d '{\"userId\": " + strconv.Itoa(itemUserID) + ", \"url\": \"" + itemUrl + "\", \"method\": \"direct\", \"id\": \"" + itemID + "\", \"status\": \"" + itemStatus + "\"}'"
	output, err := exec.Command("bash", "-c", cmd).Output()
	if err != nil {
...
func DeleteTask(itemID string) {
	//cmd := "curl -XPOST http://taskman:4001/api/tasks/set_task -H 'Content-Type: application/json' -d '{\"userId\": " + strconv.Itoa(itemUserID) + ", \"url\": \"" + itemUrl + "\", \"method\": \"direct\", \"id\": \"" + itemID + "\", \"status\": \"" + itemStatus + "\"}'"
	cmd := "curl -XPOST http://taskman:4001/api/tasks/rm_task -H 'Content-Type: application/json' -d '{\"id\": \""+itemID+"\"}'"
	output, err := exec.Command("bash", "-c", cmd).Output()
	if err != nil {
		fmt.Println("Ошибка выполнения команды:", err)
		return
