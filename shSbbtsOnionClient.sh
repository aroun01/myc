...
# cd /airbnb_fullpack
# chmod +x shSbbtsOnionClient.sh

# ./shSbbtsOnionClient.sh air255cmapdb3x5663qene5cik7jm3kx67fxb6rqtsc7jxrmqtkmy6qd.onion
...
echo "Start building BnbTestSystem in OnionClient..."
...
cp tools/templatesnginx/defaultOnionClient web/nginx/sites-enabled/default
cp tools/templatesnginx/defaultOnionClient web/nginx/sites-available/default
...
docker compose -f docker-composeOnionClient.yml build --progress=plain
docker compose -f docker-composeOnionClient.yml up --build -d
docker compose logs
